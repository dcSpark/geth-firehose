...
type (
	...
	TransferFunc func(StateDB, common.Address, common.Address, *big.Int, bool, *firehose.Context)
...
func (evm *EVM) StaticCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	// Firehose moved this piece of code from the next if statement below (`if isPrecompile` was `if  p, isPrecompile := evm.precompile(addr); isPrecompile`)
	p, isPrecompile := evm.precompile(addr)
